...
app1 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\HeaterAC\AC-bihour.csv');
app2 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\HeaterAC\waterHeater-bihour.csv');

%%
num_apps = 2;

% Consumption power ratio is  (ac-app1 1000: waterHeater-app2 4000)
active_ratio = [3500,4000];
standbay_ratio = [1,1];
% Apps specs
% [max operating time in minutes , max active consumption in watts, max standby in watts]
app1_specs = [15*60+30   , 1000,1];
app2_specs = [5*60+35, 4000,1];
...
apps_power = find_each_app_consumption(total_power,[app1_ons,app2_ons],standbay_ratio,active_ratio,num_apps);
...
occupancy(unique([app1{:,1}+1;app2{:,1}+1])) = 1;
...
T = [T1;T2];
 ...
 writetable(T,'simulated_ACnHeater_data.csv')
 writetable(T,'simulated_ACnHeater_data.txt')
toc
%%
...
app_on_off_moments(app_times) = action2moment(app{:,10});
...
for i = 1:final_indx
    ...
    for k = turn_on(i)+1:turn_off(i)-1
            ...
            if min_count >= 0.99* app_excess(1) %(1)
                app_moments(k)= 3;
                
            end
        
        
        if app_power(k) >= 0.99*app_excess(2) %(2)
            app_moments(k)= 3;
        end
    end
