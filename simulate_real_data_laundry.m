...
app1 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\Laundry-room\light-bihour.csv');
app2 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\Laundry-room\refrigerator-bihour.csv');
app3 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\Laundry-room\washingMachine-bihour.csv');
app4 = readtable('C:\Users\User\Dropbox\em3\Datasets\France\New CSV-Files\Laundry-room\tumbleDrier-bihour.csv');
%%
num_apps = 4;

% Consumption power ratio is  (light-app1 60: refrigerator-app2 180:washingmachine-app3 500:tumbledryer-app4  3000)
active_ratio = [60,180,500,3000];
standbay_ratio = [1,1,1,1];
% Apps specs
% [max operating time in minutes , max active consumption in watts, max standby in watts]
app1_specs = [8*60   , 60,1];
app2_specs = [17*60+30, 180,1];
app3_specs = [60 , 500,1];
app4_specs = [60+45 , 3000,1];
...
app2_name(:,1)="refrigerator";

app3_name = strings(length(day_time_slots),1);
app3_name(:,1)="washingMachine";

app4_name = strings(length(day_time_slots),1);
app4_name(:,1)="tumbleDryer";
...
[app4_moments,app4_ons] = find_on_off_moments(app4,day_time_slots);
...
[app4_moments,app4_ons] = fix_cluster_errors(app4_moments,app4_ons,total_power,app4_specs);

%% Appliances Power consumption (in Watts)
apps_power = find_each_app_consumption(total_power,[app1_ons,app2_ons,app3_ons,app4_ons],standbay_ratio,active_ratio,num_apps);
...
occupancy(unique([app1{:,1}+1;app2{:,1}+1;app3{:,1}+1;app4{:,1}+1])) = 1;
...
app4_moments = add_outside_momoments(app4_moments,occupancy,app4_specs,apps_power(:,4));
...
app4_moments = add_excessive_momoments(app4_moments,app4_specs,apps_power(:,4));
...
T4 = table(occupancy,app4_name,total{:,2},total{:,1},apps_power(:,4),app4_moments,'VariableNames',{'Occupancy', 'Appliance', 'Time', 'Date', 'PowerConsumption' , 'MicroMoments'});
T = [T1;T2;T3;T4];

 writetable(T,'simulated_laundry_data.csv')
 writetable(T,'simulated_laundry_data.txt')
toc
%%
...
app_on_off_moments(app_times) = action2moment(app{:,12});
